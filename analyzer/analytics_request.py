...
def application(environ, start_response):
   ...
   if  (req.method == 'GET'):
     ...
     LogInfo("Request filters: " + '\n',request_data)
     
     validated, details = checkValidation(uuid,service_name,category_name,event_name,username,proxy_username,start_date,end_date)
     if validated == 1:
       json_data, webstatus = processRequest(uuid,service_name,category_name,event_name,username,proxy_username,start_date,end_date,days,record_limit,show_history)
     elif validated == "EMPTY":
       json_data = json.dumps({'result':{'Status':'Validation failed','Report':details}},indent=4) 
...
def processRequest(uuid,service_name,category_name,event_name,username,proxy_username,start_date,end_date,days,record_limit,show_history):
 
   (QUERY_STATEMENT, QUERY_VALUES) = buildQuery(uuid,service_name,category_name,event_name,username,proxy_username,start_date,end_date,days,record_limit)
   
   LogInfo("SQL Query: ", str(QUERY_STATEMENT))
   LogInfo("SQL Query Values: ", str(QUERY_VALUES))
   
   try:

     conn = MySQLdb.connect (host = AYLT_DB_HOST,user = AYLT_DB_USERNAME,passwd = AYLT_DB_PASSWORD,db = AYLT_DB_NAME)
     cursor = conn.cursor ()
     ...
     if (len(results) > 0):
         ...
         if show_history == 1:
           ...
           if (len(histresults) > 0):
             ...
             for hrow in histresults:
               ...
               cursor.execute(QUERY_ALL_HIST_DATA % (provenance_parent_id))
	         ...
	         ph_data += "]}}"
                 ...
                 errMsg = "History Recorded but Provenance row doesn't exist" + " " + "[" + id + "]" 
                 LogError(errMsg)
                 ph_data = "{'Null'}," 
                 # notify Support
           ...
           infoMsg = "Analytics + History: Success!"
           LogInfo(infoMsg, json_data)
           curosor.close()
           webstatus = '200 OK'
       ...
       json_data = json.dumps({'result':{'Status':'Success','Details':'No records found'}}, indent=4) 
       infoMsg = "Analytics: Success! "
       LogInfo(infoMsg,json_data)
       cursor.close()
       webstatus = '200 OK'
     ...
     json_data = json.dumps({'result':{'Status':'Failed','Details':'Analytics call failed'}}, indent=4) 
     return (json_data,webstatus)


def checkValidation(uuid,service_name,category_name,event_name,username,proxy_username,start_date,end_date):
  ...
  try:
    ...
    for name, values in queryvals.iteritems():
      if values != None:
         if name == "uuid":
            ...
            if re.match ('(\d{4})[-](\d{2})[-](\d{2})$', values) != None:
               continue
            else:
               details += "'" + str(name) + "':'Failed',"
    ...
    infoMsg = "Validation: Completed"
    val_det = "Status: " + str(returnval) + " Details:  " + str(details)
    LogInfo(infoMsg,val_det)
    ...
    returnval = "EMPTY"
    infoMsg = "Validation: Failed"
    val_det = "Details: No filter was specified. " + str(e) 
    LogInfo(infoMsg,val_det)
    json_det = json.dumps({'result':{'Status':'Validation Failed','Details':val_det}}, indent=4)
    return (returnval,json_det)
